...
export function resolveWirelistTemplatePath(): string {
  const fromModule = path.resolve(
    ...
    "..",
    "data",
    "wirelist-template.xlsx"
  );
  const fromEnv = process.env.WIRELIST_TEMPLATE_PATH;
  const candidates = [fromEnv, fromModule].filter((candidate): candidate is string => Boolean(candidate));
